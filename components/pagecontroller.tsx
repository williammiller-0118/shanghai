...
const PageControls = ({ clickNext, clickFastNext, clickPrevious, clickFastPrevious, click10Previous, click10Next, part }: PageControllerProps) => {
  return (
    <div 
      className={`${part === "left" ? `${styles.leftbarPos}` : `${styles.rightbarPos}`} ${part === "left" ? "leftHidden" : ""} w-[30dvw] bottom-4 transition-opacity duration-300 bg-white bg-opacity-80 rounded-full py-1 flex gap-3 z-100`}
    >
      {/* Center Controls */}
